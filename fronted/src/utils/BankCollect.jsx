...
function BankCollect({ players, onClose, updatePlayerBalance }) {
  ...
  const handleBankCollect = (e) => {
    ...
    if (player && collectAmount > 0 && !isNaN(collectAmount) && player.balance >= collectAmount) {
      updatePlayerBalance(player.id, player.balance - collectAmount)
      onClose()
    }
  }
  ...
  const quickActions = [
    { label: 'Income Tax', amount: "2 Million" },
    { label: 'Street Repairs', amount: "40 K" },
    { label: 'Poor Tax', amount: "15 K" },
    { label: 'Hospital', amount: "100 K" }
  ]
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-linear-to-br from-emerald-800 to-emerald-900 rounded-2xl p-6 shadow-2xl border-4 border-yellow-500/30 max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-yellow-300">Bank Collects from Player</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleBankCollect} className="space-y-4">
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {quickActions.map((action, index) => (
                <button
                  ...
                  className="bg-red-600/50 hover:bg-red-600/70 text-white py-2 px-3 rounded-lg text-sm"
                >
                  {action.label}
          ...
          <button
            type="submit"
            className="w-full bg-linear-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-bold py-4 rounded-xl shadow-lg transition-all duration-200"
          >
            COLLECT FROM PLAYER
          </button>
        </form>
